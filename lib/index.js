 ...
 * @param {Array|String} options.markdownPlugins - passed as plugins to
 * markdown-it. Plugins can options as second argument.
...
module.exports = function reshapeStandard (options = {}) {
  ...
  if (!contentOpt.md) {
    ...
    // Expects options.markdownPlugins as
     if (options.markdownPlugins) {
       options.markdownPlugins.forEach(item => {
         let hasOptions = Array.isArray(item)
         let plugin = hasOptions ? item[0] : item
         let options = hasOptions ? item[1] : {}
